...
class hybfun : public green_function<double>{
  public:
    ...
    hybfun(const hybfun &rhs): green_function_vector(rhs), beta_(rhs.beta_), n_env_(rhs.n_env_) 
    {
  //    operator=(rhs);
  ...
  private:
    //Leo: a vector of Green functions for different environments
    std::vector< green_function<double> > green_function_vector;
