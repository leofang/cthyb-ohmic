...
//Leo: a vector of Green functions
n_env_(p["N_ENV"]), green_function_vector(n_env_, green_function<double>(p["N_TAU"]+1, 1, p["N_ORBITALS"]))
{
  if(!p.defined("N_TAU") || (int)(p["N_TAU"])==0) 
      throw std::invalid_argument("define parameter N_TAU, the number of hybridization time slices!");
  beta_=p["BETA"];
  //n_env_=p["N_ENV"]; //Leo: C++0x style initialization
