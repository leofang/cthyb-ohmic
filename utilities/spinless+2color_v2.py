...
#    dos = int(sys.argv[2])
    ...
    sys.exit("Invalid input! Abort\n")


initial_dir = os.getcwd()
output_dir = "output_hybridization_semicircular_low_T_more_tau_pt2"
...
if goal == 1:
    ...
    #f.write("Executable = /home/yf30/cthyb-ohmic-color/cthyb_ohmic\n")
    f.write("Executable = /home/yf30/alps/bin/hybridization\n")
    #f.write("Initialdir = /home/yf30/cthyb-ohmic-color/test_spinless+2color\n")
    #f.write("output = output/$(Cluster).$(Process).out\n")
    #f.write("error = output/$(Cluster).$(Process).err\n")
    #f.write("Log = output/$(Cluster).$(Process).log\n")
      ...
      parameters.append(
             {
               ...
               'BASENAME'                   : "hyb.param_BETA%.3f_Mu_%.1f"%(1/t, Mu), # base name of the h5 output file
               'MAX_TIME'                   : 300,                          
               'VERBOSE'                    : 0,                            
               'TEXT_OUTPUT'                : 1,                                
               'N_ENV' 			    : 1, # number of colors
               'SPINFLIP'                   : 0,
...
#               'DELTA_IN_HDF5'              : 0,                               
  ...
  for parms in parameters:
    ...
    #        # output
    #        ar=archive(output_path + parms['BASENAME']+'.out.h5','a')
    #        ar['/parameters']=parms
    #        del ar
    
    	# input & output files
            input_file = pyalps.writeInputH5Files(output_path + parms['BASENAME']+'.in.h5', [parms])
#            output_file = pyalps.writeInputH5Files(output_path + parms['BASENAME']+'.out.h5', [parms])
            ...
            for n in range(parms['N_MATSUBARA']):
               ...
               g.append(2.0/(I*w + I*sqrt(4*parms['t']**2+w**2))) # use GF with semielliptical DOS
            delta=[]
            ...
            for i in range(parms['N_ENV'] if 'N_ENV' in parms else 1):
#                input_delta=open(output_path + "delta-0" + str(i) + ".dat","w")
#                for j in range(parms["N_TAU"]+1):
#                    input_delta.write("%i "%j)
#                    for k in range(N_ORBITALS):
#                        input_delta.write("%f "%delta[j])
#                    input_delta.write("\n")
#                input_delta.close()
               ar=archive(output_path + 'DELTA' + str(i) + ".h5", 'w')
               for m in range(N_ORBITALS):
                  ar['/Delta_%i'%m]=delta
               del ar
               print "Delta H5 file generated...\n"
...
if goal == 2:
    ...
    for i in range(len(Tvalues)):
        a.append(plt.errorbar(np.array(MuValues), np.array(values[i]), errors[i], label="T=%.3f"%Tvalues[i]))
