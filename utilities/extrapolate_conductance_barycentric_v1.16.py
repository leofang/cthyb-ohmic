...
for T in Tvalues:
    sigma_file = open("giwn0_T_%.4f_%s_barycentric_with_min_max_spread.dat"%(T, output_dir), "w")
...
#          iwn = iwn[0:interpolation_order]
          ...
          for i in range(N_ENV if "N_ENV" in locals() else 1):
             ...
             a=[]
	     for j in range(6):
                 iwn_temp = iwn[0:j+10] #use 10 ~ 15 data points
                 #iwn_temp = iwn[0:j+6] #use 6~9 data points
                 giwn_temp = np.array(values[i][0:j+10])
                 s = BarycentricInterpolator(iwn_temp, giwn_temp)
                 y = s(x)
                 a.append( y[0] )
             a=np.array(a)
...
#                 s = InterpolatedUnivariateSpline(iwn, giwn, k=j+1) # spline order: 1 linear, 2 quadratic, 3 cubic ...
#                 y = s(x)
#                 a.append( y[0] )
#             a=np.array(a)
#             sigma_file.write("   %.8f   %.8f"%(a.mean(), a.max()-a.min() ))
