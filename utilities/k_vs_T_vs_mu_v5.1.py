...
values=[[[0 for T in Tvalues] for orb in range(N_ORBITALS)] for Mu in MuValues]
errors=[[[0 for T in Tvalues] for orb in range(N_ORBITALS)] for Mu in MuValues]
    ...
    #      pyalps.runApplication('../cthyb_ohmic',parms['BASENAME']+'.in.h5')
          ...
          if goal == 2:   
              ...
              for orb in range(N_ORBITALS):
                 k_mean=ar['simulation/results/order_%i/mean/value'%orb]/parms['BETA']
                 k_error=ar['simulation/results/order_%i/mean/error'%orb]/parms['BETA']
                 values[Mu_counter][orb][T_counter] = -k_mean
                 errors[Mu_counter][orb][T_counter] = k_error
              #   values[Mu_counter][orb][t_counter] = k_mean
              #   errors[Mu_counter][orb][t_counter] = k_error 
...
if goal == 2:
    ...
    plt.xlabel(r'$T^2$')
    plt.ylabel(r'$E_{kin}$')
    plt.title(r'$E_{kin}$ vs $T^2$')
    a=[]
    for i in range(len(MuValues)):
       for orb in range(N_ORBITALS):
          a.append(plt.errorbar(np.array(Tvalues)**2, np.array(values[i][orb]), errors[i][orb], \
                   label=r'orbital %i, $\mu$=%.3f'%(orb, MuValues[i])))
    plt.xlim([0, Tvalues[-1]**2])
    #plt.ylim([0.4, 0.6])
...
#    a.append(plt.plot(np.array(MuValues), theory, 'k-', linewidth=2.0, label="theory (T=0)"))
    ...
    plt.savefig('k_vs_T_vs_mu_' + output_dir + '.pdf') 
